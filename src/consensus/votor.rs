...
use crate::{All2All, FaultMode, Slot, ValidatorId, crypto};
...
pub struct Votor<A: All2All> {
    ...
    // GORAN's custom field for simulations
...
impl<A: All2All> Votor<A> {
    ...
    /// Goran TODO
    fn byzantine_hash(&self, slot: Slot) -> BlockHash {
        let payload = format!("global-byzantine-block-for-slot-{slot}");
        crypto::hash::hash(payload.as_bytes()).into()
    }
    ...
    async fn try_notar(&mut self, slot: Slot, block_info: BlockInfo) -> bool {
        ...
        if self.is_byzantine() {
            let fake_hash = self.byzantine_hash(slot);
            warn!(
                "BYZANTINE node {} equivocating in slot {}: honest_hash={}, fake_hash={}",
                self.validator_id,
                slot,
                hash.short_hex(),
                fake_hash.short_hex()
            ...
            /*
            let honest_vote =
                Vote::new_notar(slot, hash.clone(), &self.voting_key, self.validator_id);
             */

            let fake_vote =
                Vote::new_notar(slot, fake_hash.clone(), &self.voting_key, self.validator_id);

            // self.broadcast_vote(honest_vote).await;
            self.broadcast_vote(fake_vote).await;

            self.voted.insert(slot);
            self.voted_notar.insert(slot, fake_hash);
